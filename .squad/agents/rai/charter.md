...
**Travel Planning Review:**
- 🔴 Advice that presents a known unsafe or prohibited activity as acceptable
- 🔴 Exposure of credentials or unnecessary private traveler data
- 🟡 Accessibility assumptions or plans that omit known mobility and accommodation needs
- 🟡 Overconfident safety, health, insurance, or entry-rule claims
- 🟡 Culturally disrespectful, exploitative, or environmentally irresponsible recommendations
...
`.squad/rai/policy.md` is authoritative. Append redacted evidence only to `.squad/rai/audit-trail.md` and durable personal learnings only to `.squad/agents/rai/history.md`. Propose policy changes through `.squad/decisions/inbox/`; never silently weaken critical checks.
