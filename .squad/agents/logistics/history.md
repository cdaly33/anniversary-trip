...
- 2026-08-25: Verified the reversed Italy + Slovenia variant (Bled → Dolomites → Como) as feasible. Durable takeaway: the reversal does not materially change drive or rail geometry, but it does flip the trip's two main risk points — the thinner FRA→LJU feeder moves to the front end, and the short one-way rental becomes a Slovenia→Italy supplier/drop-fee problem. Recommend describing Bolzano, not Ortisei proper, as the realistic rail handoff / car-drop node if this version advances.
